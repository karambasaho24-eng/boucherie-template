...
(entries) => entries.forEach(e => { if (e.isIntersecting) { e.target.classList.add('visible'); obs.unobse
rve(e.target) } }),
{ threshold: 0.12 }
)
...
<circle cx="100" cy="100" r="80" fill="none" strokeWidth="0.6" stroke="currentColor" strokeDashar
ray="2 4" />
<text x="100" y="40" textAnchor="middle" className="stamp-text-top">QUALITÉ · HALAL · FRAÎCHEUR</
text>
...
{[
{ n: '01', title: 'Viandes sélectionnées', desc: 'Bœuf, agneau et volaille choisis avec soin aupr
ès de fournisseurs de confiance.' },
{ n: '02', title: 'Découpe artisanale', desc: 'Chaque pièce est préparée sur place par nos bouche
rs expérimentés.' },
{ n: '03', title: 'Rôtisserie fraîche', desc: 'Poulets, merguez et spécialités grillées chaque jo
ur pour emporter.' },
{ n: '04', title: 'Halal certifié', desc: 'Toutes nos viandes sont halal, rigoureusement sélectio
nnées et tracées.' },
].map((p, i) => (
...
{config?.about_text ||
"Installée au cœur du Mans, notre boucherie vous accueille chaque jour avec des produits frais,
une découpe soignée et le sourire."}
</p>
...
.hero-btn-main { border-color: var(--color-on-ink-fixed); background: var(--color-on-ink-fixed); color:
var(--color-ink-fixed); }
...
.hero-call-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--color-red); flex-shrink
: 0; }
...
.info-label { font-family: var(--font-mono); font-size: 10px; text-transform: uppercase; letter-spacing
: 1.5px; color: var(--color-text-muted); margin-bottom: 4px; }
...
.pillar-desc { font-size: 14px; color: var(--color-text-muted); margin: 0; line-height: 1.65; max-width
: 480px; }
...
.ambiance-text { font-size: 16px; color: var(--color-on-ink-fixed-dim); opacity: 0.9; line-height: 1.8;
margin: 0; }
...
.info-item { flex-direction: column; gap: 10px; padding: 32px 28px; border-bottom: none; border-right
