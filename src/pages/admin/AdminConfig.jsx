...
import { fetchSiteConfig, fetchStripeSecrets, updateSiteConfig, uploadSiteImage, uploadSiteVideo } from '../../
lib/api'
const THEME_COLORS = [
...
<textarea className="textarea" name="hero_subtitle" rows={2} value={config.hero_subtitle || ''} onC
hange={handleChange} />
</div>
</div>
<div className="config-block">
...
<input className="input" name="opening_hours" value={config.opening_hours || ''} onChange={handleCh
ange} placeholder="09:30 - 19:30" />
</div>
</div>
<div className="config-block">
...
<input ref={bannerRef} type="file" accept="image/*" capture="environment" style={{ display: 'none' }}
onChange={handleBannerUpload} />
<button className="btn btn-ghost btn-sm" onClick={() => bannerRef.current.click()} disabled={uploadin
g}>
{uploading ? 'Upload…' : 'Prendre une photo / Choisir une image'}
</button>
{config.banner_image && (
<button className="btn btn-danger btn-sm banner-delete-btn" onClick={() => setConfig((c) => ({ ...c
, banner_image: '' }))}>
Supprimer
</button>
)}
...
Si une vidéo est ajoutée, elle remplace l'image bannière en fond de page d'accueil (lecture automat
ique, en boucle, sans son). Format MP4 recommandé, 30 Mo max.
</p>
{config.banner_video && (
<video src={config.banner_video} className="banner-preview" autoPlay loop muted playsInline />
)}
<input ref={bannerVideoRef} type="file" accept="video/*" style={{ display: 'none' }} onChange={handle
BannerVideoUpload} />
<button className="btn btn-ghost btn-sm" onClick={() => bannerVideoRef.current.click()} disabled={upl
oading}>
...
<input ref={logoRef} type="file" accept="image/*" capture="environment" style={{ display: 'none' }} o
nChange={handleLogoUpload} />
...
<p className="text-muted" style={{ fontSize: 12.5, marginTop: -6, marginBottom: 12 }}>
Petite icône affichée dans l'onglet du navigateur. Format carré recommandé (ex : 64×64px, PNG ou IC
O).
</p>
{config.favicon_url && (
<img src={config.favicon_url} alt="Favicon" className="favicon-preview" />
)}
<input ref={faviconRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={handleFavi
conUpload} />
<button className="btn btn-ghost btn-sm" onClick={() => faviconRef.current.click()} disabled={uploadi
ng}>
...
<input className="input" name="about_title" value={config.about_title || ''} onChange={handleChange
} placeholder="Notre histoire" />
</div>
<div className="field">
<label>Texte</label>
<textarea className="textarea" name="about_text" rows={5} value={config.about_text || ''} onChange=
{handleChange} />
</div>
</div>
<div className="config-block">
...
sans que tu aies besoin de le faire toi-même. En mode manuel, tu confirmes chaque commande à la mai
n.
</p>
...
<input className="input" name="whatsapp_number" value={config.whatsapp_number || ''} onChange={hand
leChange} placeholder="+33243410951" />
</div>
<div className="field">
<label>Mode de commande</label>
<select className="select" name="order_mode" value={config.order_mode || 'both'} onChange={handleCh
ange}>
...
<p className="delivery-toggle-desc">
Lorsqu'activé, le client peut payer en ligne une commande confirmée (que ce soit via le site ou
via WhatsApp), ou choisir de payer sur place.
</p>
</div>
<label className="toggle-switch">
<input
...
Utilisez les clés <em>Test</em> pour vos essais, puis les clés <em>Live</em> pour les vrais paiemen
ts.
</p>
<div className="field">
<label>Mode</label>
...
<li>URL : <code className="stripe-code">{window.location.origin.replace('admin','')}/functions/v1
/stripe-webhook</code></li>
<li>Événements à écouter : <code className="stripe-code">checkout.session.completed</code> et <co
de className="stripe-code">checkout.session.expired</code></li>
...
<style>{`
.section-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 24
px; }
.section-header h2 { margin: 0; font-family: var(--font-display); font-weight: 600; font-size: 22px; le
tter-spacing: -0.3px; }
.config-grid { display: grid; gap: 1px; grid-template-columns: 1fr; background: var(--color-border); bo
rder: 1px solid var(--color-border); }
.config-block { padding: 24px; background: var(--color-surface); }
.config-block h4 { font-family: var(--font-mono); font-size: 11px; font-weight: 700; letter-spacing: 1.
5px; text-transform: uppercase; margin: 0 0 18px; color: var(--color-red); }
...
.banner-preview { width: 100%; height: 140px; object-fit: cover; margin-bottom: 12px; border: 1px solid
var(--color-border); }
.logo-preview { width: 72px; height: 72px; object-fit: cover; margin-bottom: 12px; border: 1px solid va
r(--color-border); border-radius: 6px; }
.favicon-preview { width: 40px; height: 40px; object-fit: cover; margin-bottom: 12px; border: 1px solid
var(--color-border); border-radius: 6px; }
.theme-swatches { display: flex; flex-wrap: wrap; gap: 8px; }
.theme-swatch-btn { display: flex; align-items: center; gap: 8px; padding: 8px 12px; border: 1px solid
var(--color-border); background: var(--color-paper-dim); transition: all 0.2s; }
.theme-swatch-btn:hover { border-color: var(--color-text-muted); }
.theme-swatch-btn.active { border-color: var(--color-text); background: var(--color-surface); box-shado
w: inset 0 0 0 1px var(--color-text); }
.theme-swatch-dot { width: 16px; height: 16px; border-radius: 50%; flex-shrink: 0; border: 1px solid rg
ba(0,0,0,0.12); }
...
.whatsapp-preview { margin-top: 16px; background: var(--color-paper-dim); padding: 12px 14px; border: 1
px solid var(--color-border); }
.whatsapp-preview-label { font-family: var(--font-mono); font-size: 10.5px; letter-spacing: 0.5px; text
-transform: uppercase; color: var(--color-text-muted); margin: 0 0 8px; }
.whatsapp-preview-text { font-family: var(--font-mono); font-size: 12px; white-space: pre-wrap; margin:
0; color: var(--color-text); line-height: 1.6; }
.delivery-toggle-row { display: flex; align-items: flex-start; justify-content: space-between; gap: 16p
...
.toggle-slider { position: absolute; inset: 0; background: var(--color-border-dark); transition: 0.3s;
border-radius: 24px; cursor: pointer; }
.toggle-slider:before { content: ''; position: absolute; width: 18px; height: 18px; left: 3px; bottom:
3px; background: white; transition: 0.3s; border-radius: 50%; }
...
.status-off { background: var(--color-paper-dim); color: var(--color-text-muted); border: 1px solid var
(--color-border); }
.field { display: flex; flex-direction: column; gap: 6px; margin-bottom: 14px; }
.field label { font-size: 11px; font-weight: 700; font-family: var(--font-mono); text-transform: upperc
ase; letter-spacing: 0.8px; color: var(--color-text-muted); }
.config-error { color: var(--color-red); margin-top: 16px; font-size: 13px; }
/* Stripe */
.stripe-block { grid-column: 1 / -1; }
.stripe-intro { font-size: 12.5px; color: var(--color-text-muted); margin: -6px 0 18px; line-height: 1.
55; }
.stripe-mode-toggle { display: flex; gap: 0; border: 1px solid var(--color-border); width: fit-content;
margin-bottom: 4px; }
.stripe-mode-btn { padding: 8px 18px; font-size: 12px; font-weight: 700; border: none; background: var(
--color-paper-dim); color: var(--color-text-muted); cursor: pointer; transition: all 0.2s; }
...
.stripe-help-box { margin-top: 20px; background: var(--color-paper-dim); border: 1px solid var(--colorborder); padding: 16px 18px; }
.stripe-help-title { font-weight: 700; font-size: 13px; margin: 0 0 10px; }
.stripe-help-steps { margin: 0; padding-left: 18px; display: flex; flex-direction: column; gap: 6px; fo
nt-size: 12.5px; color: var(--color-text-muted); line-height: 1.5; }
.stripe-help-steps strong { color: var(--color-text); }
.stripe-code { font-family: var(--font-mono); font-size: 11px; background: var(--color-border); padding
: 1px 5px; }
